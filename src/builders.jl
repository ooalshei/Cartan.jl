...
function hamiltonian(model::AbstractString, n::Integer, couplings::AbstractVector{<:Real}=[1.0], ::Type{T}=UInt; nf::Integer=0, pbc::Bool=false) where {T<:Unsigned}
    ...
    elseif uppercase(model) == "TFXY"
        if length(couplings) != 2
            ...
            strings = append!(generatexx(n, T, pbc=pbc), generateyy(n, T, pbc=pbc), generatez(n, T))
            coefficients = fill(-couplings[1], length(strings))
            coefficients[end-n+1:end] *= couplings[2]
        end
    ...
    elseif uppercase(model) == "CFIM"
        if length(couplings) != 3
            throw(ArgumentError("Incorrect number of couplings. Expected 3 (J,gh,gl), got $(length(couplings)). H = -J(XX + gh Z + gl X)"))
    ...
    elseif uppercase(model) == "HEISENBERG"
        ...
        strings = append!(generatexx(n, T, pbc=pbc), generateyy(n, T, pbc=pbc), generatezz(n, T, pbc=pbc))
        coefficients = fill(-couplings[1], length(strings))

    elseif uppercase(model) == "XXZ"
        length(couplings) == 2 || throw(ArgumentError("Incorrect number of couplings. Expected 2 (J,Δ), got $(length(couplings)). H = -J(XX + YY + Δ ZZ)"))
        strings = append!(generatexx(n, T, pbc=pbc), generateyy(n, T, pbc=pbc), generatezz(n, T, pbc=pbc))
        coefficients = fill(-couplings[1], length(strings))
        coefficients[2*length(strings)÷3+1:end] *= couplings[2]

    elseif uppercase(model) == "GN"
        length(couplings) == 3 || throw(ArgumentError("Incorrect number of couplings. Expected 3 (μ,G,m), got $(length(couplings)). H = (1-2μ)(YX - XY) + (2m-4nf nG)Z - 4G ZZ "))
        nf > 0 || throw(ArgumentError("Number of flavors must be greater than 0."))
        strings = generateyx(nf * n, T)
        setdiff!(strings, T[exp2(i*n-1) + exp2(i*n) + exp2(i*n + n*nf) for i in 1:nf-1])
        temp = generatexy(nf * n, T)
        setdiff!(temp, T[exp2(i*n-1) + exp2(i*n) + exp2(i*n + n*nf-1) for i in 1:nf-1])
        append!(strings, temp)
