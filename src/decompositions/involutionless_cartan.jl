...
function involutionlessdecomp(paulis::PauliList)
    ...
    while !contradiction
        for i in eachindex(algebra)[end:-1:initialind]
            ...
            length(algebra) == length(unique!(x -> x.first, algebra)) || (contradiction = true)
        end
        if finalind == length(algebra)
            alg = PauliList(first.(algebra), paulis.qubits, iscopy=false)
            contradiction && return Dict('g' => alg, 'h' => subalgfind(alg), 'k' => nothing, 'm' => nothing)
        ...
        contradiction && (algebra = PauliList(first.(algebra), paulis.qubits, iscopy=false); break)
    end
    ...
    while true
        ...
        finalind == length(algebra) && return Dict('g' => algebra, 'h' => subalgfind(algebra), 'k' => nothing, 'm' => nothing)
        initialind = finalind + 1
        finalind = length(algebra)
    end
end
